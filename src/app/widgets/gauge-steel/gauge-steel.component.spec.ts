...
describe('GaugeSteelComponent', () => {
  ...
  it('builds a Compass for the compass subType, with the card rotating under a fixed pointer', () => {
    const built: { id: string; options: Record<string, unknown> }[] = [];
    const steel = (globalThis as unknown as { steelseries: Record<string, unknown> }).steelseries;
    steel.Compass = vi.fn(function (this: FakeGauge, id: string, options: Record<string, unknown>) {
      built.push({ id, options });
      this.setValue = vi.fn();
      this.setValueAnimated = vi.fn();
    });

    fixture.componentRef.setInput('widgetUUID', 'uuid-compass');
    fixture.componentRef.setInput('subType', 'compass');

    const internals = component as unknown as { startGauge: (f?: boolean) => void };
    internals.startGauge(true);

    expect(built).toHaveLength(1);
    expect(built[0].id).toBe('uuid-compass');
    // The defaults are the picked design: a turning card read against the pointer at the top, with
    // the degree scale printed and the ornamental rose left off.
    expect(built[0].options.rotateFace).toBe(true);
    expect(built[0].options.degreeScale).toBe(true);
    expect(built[0].options.roseVisible).toBe(false);
  });

  it('passes the compass card options through as configured', () => {
    fixture.componentRef.setInput('subType', 'compass');
    fixture.componentRef.setInput('rotateFace', false);
    fixture.componentRef.setInput('degreeScale', false);
    fixture.componentRef.setInput('roseVisible', true);

    const internals = component as unknown as {
      buildOptions: () => void;
      gaugeOptions: Record<string, unknown>;
    };
    internals.buildOptions();

    expect(internals.gaugeOptions.rotateFace).toBe(false);
    expect(internals.gaugeOptions.degreeScale).toBe(false);
    expect(internals.gaugeOptions.roseVisible).toBe(true);
  });

  it('sizes the compass square, like the radial, and drops the linear geometry keys', () => {
    fixture.componentRef.setInput('subType', 'compass');

    const internals = component as unknown as {
      lastRect: { width: number; height: number } | null;
      applyGeometry: () => string;
      gaugeOptions: Record<string, unknown>;
    };
    internals.gaugeOptions['width'] = 400;
    internals.gaugeOptions['height'] = 120;
    internals.lastRect = { width: 400, height: 260 };

    expect(internals.applyGeometry()).toBe('compass:260');
    expect(internals.gaugeOptions.size).toBe(260);
    expect(internals.gaugeOptions.width).toBeUndefined();
    expect(internals.gaugeOptions.height).toBeUndefined();
  });

  it('survives a rename on a gauge class that carries no title', () => {
    const steel = (globalThis as unknown as { steelseries: Record<string, unknown> }).steelseries;
    const animated: number[] = [];
    steel.Compass = vi.fn(function (this: FakeGauge) {
      this.setValue = vi.fn();
      this.setValueAnimated = (v: number) => { animated.push(v); };
    });

    fixture.componentRef.setInput('widgetUUID', 'uuid-compass-title');
    fixture.componentRef.setInput('subType', 'compass');
    fixture.componentRef.setInput('value', 47);
    ...
    // The Compass has no setTitleString. Calling it would throw out of ngOnChanges and take the
    // value update batched with it down too — the heading would freeze on a rename.
    fixture.componentRef.setInput('value', 132);
    expect(() => internals.ngOnChanges({
      title: new SimpleChange('Heading', 'Ship Heading', false),
      value: new SimpleChange(47, 132, false)
    })).not.toThrow();
    expect(animated).toEqual([132]);
  });
  it('holds the last frame when the value goes back to null instead of swinging to zero', () => {
    const animated: (number | null)[] = [];
    const steel = (globalThis as unknown as { steelseries: Record<string, unknown> }).steelseries;
    steel.Compass = vi.fn(function (this: FakeGauge) {
      this.setValue = vi.fn();
      this.setValueAnimated = (v: number) => { animated.push(v); };
    ...
    fixture.componentRef.setInput('widgetUUID', 'uuid-compass-null');
    fixture.componentRef.setInput('subType', 'compass');
    fixture.componentRef.setInput('value', 47);
    ...
    // The compass clears its reading when re-pointed at another path. Passing that null on would
    // read as a heading of due north — the library coerces it to 0.
    fixture.componentRef.setInput('value', null);
    internals.ngOnChanges({ value: new SimpleChange(47, null, false) });
